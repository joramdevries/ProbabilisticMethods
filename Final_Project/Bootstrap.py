...
def bootstrap_function(WindData, Nbootstrap, plots):
    ...
    N1month = int(1*30*24*6) # taking an interval for 1 year with 365 days with 24 hours with 6 times 10 minute intervals
    
    
    BootstrapSize = N1month
    ...
    for n in range(Nbootstrap):
        ...
        Bsample = np.random.randint(low = 0, high = len(WindData) - N1month)
        
        # Use .iloc to access the row by index
        start_month = WindData['Timestamp'].iloc[Bsample]
        # Add one year to the start_year
        end_month = start_month + timedelta(days=30)
        ...
        random_month = WindData.loc[(WindData['Timestamp'] >= start_month) & (WindData['Timestamp'] <= end_month)]
        
        month_wsp_mean = np.mean(random_month['Wsp'])
        ...
        BootstrapSample[n] = month_wsp_mean #Bsample_final
...
if __name__ == "__main__":
    ...
    WindData = pd.read_csv('lidar_data_2hz.csv')

    WindData['Timestamp'] = pd.to_datetime(WindData['TimeStamp'], format="%Y-%m-%d %H:%M:%S.%f") #format='%Y%m%d%H%M')

    WindData['Month'] = WindData['Timestamp'].dt.month
    
    WindData['Wsp'] = WindData['Wsp_44m']
    ...
    U_YMP = np.zeros(len(months))
    
    for y in range(len(months)):
        
        Winddata_1_year = WindData[WindData['Month'] == int(months[y])]
