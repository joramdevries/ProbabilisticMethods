...
def LSTM_function_time_lag(data, input_data, output, model_name, n_lag, n_out):
...
def LSTM_function(data, input_data, output, model_name):
    ...
    if model_name == 'lidar4moredata_RELU' or model_name == 'lidar4moredata_RELU_MAE':
    ...
    if time_lag:
        ...
        for n_lag in lags:
            #for n_out in outs:
                ...
                #print(n_out)
                n_out = 1
                model = f"timelag{n_lag}_forecast{n_out}"
                if training_model:
                    LSTM_function_time_lag(data, input_data, outputs,model,n_lag,n_out)
